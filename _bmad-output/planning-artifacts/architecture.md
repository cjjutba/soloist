...
| Auth | **Better Auth** (core; email/password) | ^1.6 | Email/password + email verification, password policy, sessions, rate limiting. Tenant = custom `tenants` row owned by the Freelancer; member/invite identity is app-level (`ClientAccess` + custom Engagement Invitation). _(Org plugin dropped from v1 — refined Story 1.3.)_ |
...
**Decision: Better Auth (email/password); path + role authorization on a single domain, enforced in the role guard and the data layer.** _(revised 2026-06-06 — path-based routing; see sprint-change-proposal-2026-06-06.md)_ _(refined 2026-06-06, Story 1.3 — v1 ships Better Auth **core** only, **not** the organization plugin: the Tenant is the custom `tenants` table owned by the Freelancer via `owner_user_id` (UNIQUE = one Tenant per freelancer). The org plugin's members/invitations are unused in v1 — all member/invite identity is already app-level (`ClientAccess` + custom Engagement Invitation) — and adopting it would force `tenants.id` from uuid→text, breaking the verified uuid RLS core. The org plugin remains a deferred upgrade if multi-member teams ever land.)_
...
- **Mapping:** `Tenant = the custom `tenants` row` (owner = the Freelancer, via `owner_user_id`). _(was "Tenant = Organization"; refined Story 1.3 — see Decision note above.)_ **Client identities** are `User`s linked to a single Engagement via `ClientAccess` (app-level, because Client scope is Engagement-grained, finer than org membership). The Client invite (FR-5) uses a custom Engagement-scoped `Invitation` (unique, hashed, expiring token) → on accept, creates the `User` (or links existing) + `ClientAccess`, then routes to Onboarding.
