...
- **Activity/notifications bell:** the existing `/api/notifications` route is **client-only** (it returns 403 for a freelancer — the notifications table is addressed to the client recipient), so the cockpit cannot reuse it. The bell instead surfaces a **real, freelancer-derived attention feed** built from the single cached dashboard read (§5.3 — no extra queries per render): the engagements that need action — those with a curation backlog or unread client messages — each linking to that engagement, with an unread-dot when the total is non-zero. Outstanding-invoice totals and recent published ship-updates are surfaced on the **Overview** (the KPI cards, Outstanding-invoices, and Recent-updates panels) rather than the bell, so the persistent chrome stays at one query per render. Server-computed; refreshes on navigation. (Cross-engagement push-live refresh is deferred — see §5.3.)
